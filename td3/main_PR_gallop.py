...
if __name__ == "__main__":
	...
	parser.add_argument("--expl_noise", default=np.exp(-1))                # Std of Gaussian exploration noise
	...
	if not os.path.exists("./PR_gallop_results"):
		os.makedirs("./PR_gallop_results")

	if args.save_model and not os.path.exists("./PR_gallop_models"):
		os.makedirs("./PR_gallop_models")

	env = rexPeriodicRewardEnv(render=False, gait_type='gallop')
	...
	for t in range(int(args.max_timesteps)):
		...
		if (t + 1) % args.eval_freq == 0:
			...
			np.save(f"./PR_gallop_results/{file_name}", evaluations)
			if args.save_model: 
				policy.save(f"./PR_gallop_models/{file_name}")
